(ns account-ledger.shared.logic.contracts-test
...
(deftest dates-and-query-schemas
  (let [daily {:account/id "ACC-001" :mode :daily :run-day 2 :reference-day 1 :booking-cutoff 1}
    ...
    (is (s/valid? ::schemas/calculation daily))
    (is (not (s/valid? ::schemas/calculation (assoc daily :interest-only? true))))
    (is (not (s/valid? ::schemas/calculation (assoc review :interest-only? true))))
    (is (not (s/valid? ::schemas/calculation
                       (assoc review :input-through-event-counter 2 :interest-only? :yes))))
    (is (not (s/valid? ::schemas/calculation (assoc daily :booking-cutoff 2))))
    (is (s/valid? ::schemas/calculation review))
    (is (not (s/valid? ::schemas/calculation (assoc review :input-through-event-counter 2))))
    (is (s/valid? ::schemas/calculation (assoc review :input-through-event-counter 2 :interest-only? true)))
    (is (not (s/valid? ::schemas/calculation (assoc review :through-day 6))))
    (is (s/valid? ::schemas/settlement settlement))
    (is (not (s/valid? ::schemas/settlement (assoc settlement :period/end-day 6))))
    (is (s/valid? ::schemas/ledger-query {:account/id "ACC-001" :value-through-day 2
