(ns account-ledger.authorization.logic.core-test
...
(defn initial [] (logic/initial-state (contracts/normalize-config fixtures/config)))

(defn apply-commands [state commands]
  (reduce (fn [s command] (first (logic/submit s command))) state commands))
  ...
  (doseq [[account-id currency funds over zero minor-unit]
    ...
    (testing (str currency " uses its own minor unit at the availability boundary")
      (let [account {:account/id account-id :money/currency currency}
            ...
            [exact approved] (logic/submit funded
            ...
            [declined refused] (logic/submit funded declined-command)]
        ...
        (let [more-funds (apply-commands declined [(fixtures/command "later" :credit 100M 2 account)])
              [unchanged duplicate] (logic/submit more-funds {:transaction/id "over" :money/amount :malformed})]
      ...
      (testing "zero funds cannot support even the smallest hold"
        (let [before (initial)
              ...
              [after result] (logic/submit before command)]
...
(deftest known-identity-precedes-every-payload-check
  (let [[recorded original] (logic/submit (initial) (fixtures/command "same" :credit 10.00M 1))]
    (doseq [duplicate [{:transaction/id "same"}
      ...
      (let [[state result] (logic/submit recorded duplicate)]
        (is (= recorded state))
...
(deftest partial-settlement-and-explicit-release
  (let [funded (apply-commands (initial)
        ...
        [partial result] (logic/submit funded (fixtures/command "partial" :settlement 15.00M 1
                                                                {:authorization/id "A" :settlement/final? false}))
        [released release-result] (logic/submit partial (fixtures/command "release" :release 5.00M 1
                                                                          {:authorization/id "A"}))]
    ...
    (let [[unchanged invalid] (logic/submit partial (fixtures/command "over-release" :release 5.01M 1
                                                                                      {:authorization/id "A"}))]
...
(deftest confirmed-settlement-debits-without-sufficient-funds-or-live-hold
  (let [[unmatched result] (logic/submit (initial) (fixtures/command "missing" :settlement 180.00M 1
  ...
  (let [held (apply-commands (initial) [(fixtures/command "fund" :credit 20.00M 1)
                                       (fixtures/command "hold" :authorization 20.00M 1 {:authorization/id "A"})])
        [settled result] (logic/submit held (fixtures/command "larger" :settlement 30.00M 1
                                                               {:authorization/id "A" :settlement/final? false}))]
    (is (= :recorded (:outcome result)))
    ...
    (let [[again recorded] (logic/submit settled (fixtures/command "after-settled" :settlement 2.00M 1
...
(deftest source-version-is-account-specific-and-unrecorded-attempts-retain-identity
  (let [funded (apply-commands (initial) [(fixtures/command "fund" :credit 10.00M 1)])
        [stale result] (logic/submit funded (financial-command "system/payment" 1.00M 0))]
    (is (= funded stale))
    ...
    (let [declined (apply-commands stale [(fixtures/command "decline" :authorization 10.01M 2
          ...
          [paid result] (logic/submit other (financial-command "system/payment" 0.10M 1))
          event (:recorded/event result)
          [again duplicate] (logic/submit paid {:transaction/id "system/payment" :source-event-counter 0
  ...
  (let [before (initial)
    ...
    (doseq [key [:settlement/id :period/end-day :booking-cutoff]]
      (let [invalid (dissoc command key)
            [unchanged rejected] (logic/submit before invalid)]
        (is (= :invalid (:outcome rejected)))
        (is (= :invalid-interest-settlement (:reason rejected)))
        (is (= before unchanged))
        (let [[recorded result] (logic/submit unchanged command)
              [duplicate-state duplicate] (logic/submit recorded invalid)]
          (is (= :recorded (:outcome result)))
...
(deftest reversal-restores-principal-once-with-supplied-dates
  (let [state (apply-commands (initial) (take 8 fixtures/events))
        [reversed result] (logic/submit state (nth fixtures/events 8))
        event (:recorded/event result)]
    (is (= :recorded (:outcome result)))
    ...
    (let [[same duplicate] (logic/submit reversed {:transaction/id "E9"})
          [unchanged repeated] (logic/submit reversed (assoc (nth fixtures/events 8) :transaction/id "again"))]
      (is (= :duplicate (:outcome duplicate)))
    ...
    (doseq [[command reason]
      ...
      (let [[unchanged result] (logic/submit state command)]
...
(deftest unrelated-credit-metadata-cannot-consume-a-principal-reversal
  (let [credited (apply-commands (initial) [(fixtures/command "A" :credit 100.00M 1)])
        [before credit-result] (logic/submit credited
        ...
        [reversed result] (logic/submit before reversal)
        [unchanged repeated] (logic/submit reversed (assoc reversal :transaction/id "R-again"))]
...
(deftest installment-credit-is-one-financial-effect-and-one-snapshot
  (let [[state result] (logic/submit (initial) (last fixtures/events))
        event (:recorded/event result)]
    (is (= :recorded (:outcome result)))
...
(deftest release-can-free-a-partial-amount-without-posting-money
  (let [held (apply-commands (initial)
                             [(fixtures/command "fund" :credit 20.00M 1)
                              (fixtures/command "hold" :authorization 20.00M 1 {:authorization/id "A"})])
        [partial result] (logic/submit held (fixtures/command "release-part" :release 5.00M 1
                                                                {:authorization/id "A"}))
        [released _] (logic/submit partial (fixtures/command "release-rest" :release 15.00M 1
                                                                  {:authorization/id "A"}))]
    ...
    (doseq [[state command reason]
      ...
      (let [[unchanged result] (logic/submit state command)]
        ...
        (is (= reason (:reason result)))
        (is (= state unchanged))))
    (let [[settled result] (logic/submit released (fixtures/command "after-release" :settlement 25.00M 1
                                                                                        {:authorization/id "A" :settlement/final? true}))]
...
(deftest reversal-of-credit-and-optional-exact-amount
  (let [[credit _] (logic/submit (initial) (fixtures/command "credit" :credit 10.000M 1
                                                                          {:account/id "ACC-002" :money/currency :BHD}))
        ...
        [reversed result] (logic/submit credit reversal)]
    (is (= :recorded (:outcome result)))
    ...
    (doseq [[amount reason] [[10.0001M :excess-precision] [10.0 :invalid-amount] [nil :invalid-amount]]]
      (let [[unchanged result] (logic/submit credit (assoc reversal :money/amount amount))]
        (is (= credit unchanged))
        (is (= :invalid (:outcome result)))
        (is (= reason (:reason result)))))))
...
(deftest financial-debit-preserves-fee-contract-even-into-negative-funds
  (let [command (fee-command)
        [state result] (logic/submit (initial) command)
        event (:recorded/event result)]
    (is (= :recorded (:outcome result)))
    ...
    (let [reversal (dissoc (fixtures/command "reverse-fee" :reversal nil 7 {:reversal/of "system/fee"}) :money/amount)
          [unchanged rejected] (logic/submit state reversal)]
      (is (= state unchanged))
...
(deftest invalid-fee-assessments-cannot-record-effects-or-reserve-identity
  (let [before (initial)
    ...
    (doseq [invalid [(dissoc command :fee/assessment)
      ...
      (let [[unchanged rejected] (logic/submit before invalid)]
        (is (= :invalid (:outcome rejected)))
...
(deftest delivery-acknowledgement-is-a-pure-idempotent-transition
  (let [[recorded _] (logic/submit (initial) (first fixtures/events))
        before (logic/pending-deliveries recorded)
        id (:delivery/id (first before))
        [acknowledged result] (logic/ack-delivery recorded id)
        [repeated repeated-result] (logic/ack-delivery acknowledged id)
        [unknown unknown-result] (logic/ack-delivery acknowledged :unknown)]
    (is (= {:delivery/id id :acknowledged? true} result repeated-result))
    (is (= {:delivery/id :unknown :acknowledged? false :reason :unknown-delivery} unknown-result))
    (is (= acknowledged repeated unknown))
    (is (= (subvec before 1) (logic/pending-deliveries acknowledged)))
    (is (= before (logic/pending-deliveries recorded)))
    (is (= (logic/snapshot recorded "ACC-001") (logic/snapshot acknowledged "ACC-001")))
    (is (= (logic/history recorded "ACC-001") (logic/history acknowledged "ACC-001")))
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Unknown account"
                          (logic/snapshot acknowledged "missing")))
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Unknown account"
                          (logic/history acknowledged "missing")))))
