...
* An exception or unknown result preserves the exact pending command. Retrying the same settlement resends that saved command; calculation resumes its pending fee first. Another Yield fee command or interest settlement, including a new zero settlement, waits. Principal credits and debits remain accepted by Authorization. Pure accruals can continue without changing the earlier intent.
...
At the user's subsequent request, the repo includes one concise [architecture document](docs/architecture.md), [tradeoffs](docs/trade-offs.md) and [production considerations](docs/production-considerations.md), with one combined PDF limited to four pages. Architecture is the sole source for its PDF section; detailed rules and interface contracts are linked in their dedicated documents to avoid maintaining duplicate architecture descriptions. Production recommendations and the cited CBUAE consumer disclosure/statement requirements are explicitly separated from implemented behavior and the exercise's adopted numerical rules.
