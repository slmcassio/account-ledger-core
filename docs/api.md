...
Run these APIs in one JVM. Module constructors return opaque handles. Application callers use each module's `ports.api-server` namespace. Pure rules and state transitions live in `logic`, internal map schemas in `model.models`, and local storage in `db.memory`. Outgoing calls belong to `ports.api-client`; logic calls neither APIs nor storage. `!` marks mutation or external delivery. Queries read current module state, return immutable values and cause no delivery, recalculation or posting.
