(ns account-ledger.ledger.domain
...
(defn- normalize-movement [config movement]
  ...
  (let [submitted-fields (apply dissoc movement contracts/computed-event-fields)
...
(defn account-balance
  ...
  [state query]
  (when-not (s/valid? ::contracts/ledger-query query)
